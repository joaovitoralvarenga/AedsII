...
public class PesquisaSequencial {
    public static boolean BuscaSequencial(String title, Show[] shows, int[] comparacoes) {        //Declara-se um array de comparações, a fim de passs-lo por referência
        boolean encontrado = false;                                                               //para que possa ser acessado e moldado na "main"; 
        comparacoes[0] = 0; 
    ...
    public static void main(String[] args) {
        ...
        while(!Show.ehFim(entrada)) {
          ...
          entrada = scanner.nextLine();
            ...
            while(!Show.ehFim(entrada)) {
                boolean achado = BuscaSequencial(entrada, shows, comparacoes);        
            ...
            double tempo = (fim - inicio) / 1e6;       //Determina o tempo de execução do código em "ms", para tornar mais preciso a visualização.


            try {
                FileWriter arquivo = new FileWriter("matricula_sequencial.txt");
                arquivo.write("872857" + '\t' + tempo + '\t' + comparacoes[0]);
                arquivo.close();
            } catch(IOException e) {
