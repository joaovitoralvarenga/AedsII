...
void inserir(Lista *lista, Show show, int pos) {
    if(lista->n >= MAX_SHOWS || pos < 0 || pos > lista->n) {
        fprintf(stderr, "Lista cheia\n");
...
while(!ehFim(entrada)) {
    ...
    inserirFim(&lista,clone(&shows[id - 1]));
    scanf("%s",entrada);
}
...
for(int i=0;i<n;i++) {
    ...
    if(strncmp(opp, "II", 2) == 0) {
        int id;
        sscanf(opp + 3, "s%d", &id);
        inserirInicio(&lista, clone(&shows[id - 1])); 
        ...
        inserir(&lista, clone(&shows[id - 1]), pos);
