...
void incializaPilha(Pilha *pilha) {
    pilha->n = 0;
}

void inserir(Pilha *pilha, Show show) {
    if(pilha->n > MAX_SHOWS) {
        fprintf(stderr, "Pilha cheia!\n");
    }

    pilha->array[pilha->n] = show;                        //Define-se a inserção de registros na última posição do conjunto, a fim de respeitar a estrutura da pilha.
    pilha->n++;
}
...
Show remover(Pilha *pilha) {
    ...
    pilha->n--;                                         //Assim como a inserção, a remoção só pode ser feita no último elemento, dessa forma se decrementa o valor da "capacidada" da lista
    return pilha->array[pilha->n];                      //e retorna esse elemento.
} 
...
while(!ehFim(entrada)) {
    ...
    inserir(&pilha,clone(&shows[id - 1]));
    scanf("%s",entrada);
}
